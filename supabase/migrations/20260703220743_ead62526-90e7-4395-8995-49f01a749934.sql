...
ALTER TABLE public.orbit_advisor_suggestions
  ADD COLUMN IF NOT EXISTS dedupe_key text,
  ADD COLUMN IF NOT EXISTS expires_at timestamptz;

CREATE UNIQUE INDEX IF NOT EXISTS uq_advisor_suggestions_dedupe_pending
  ON public.orbit_advisor_suggestions (empresa_id, dedupe_key)
...
REVOKE ALL ON FUNCTION public.get_advisor_snapshot_admin(uuid) FROM public;
REVOKE ALL ON FUNCTION public.get_advisor_snapshot_admin(uuid) FROM anon;
REVOKE ALL ON FUNCTION public.get_advisor_snapshot_admin(uuid) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.get_advisor_snapshot_admin(uuid) TO service_role;

CREATE OR REPLACE FUNCTION public.list_advisor_scan_targets()
RETURNS TABLE (empresa_id uuid, nome text, plano text)
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path = public
AS $$
...
REVOKE ALL ON FUNCTION public.list_advisor_scan_targets() FROM public;
REVOKE ALL ON FUNCTION public.list_advisor_scan_targets() FROM anon;
REVOKE ALL ON FUNCTION public.list_advisor_scan_targets() FROM authenticated;
GRANT EXECUTE ON FUNCTION public.list_advisor_scan_targets() TO service_role;
