...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Verifica que o usuário pertence à empresa (ou é super_admin)
    const svc = createClient(
      Deno.env.get("SUPABASE_URL")!,
      Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!,
    );
    const { data: profile } = await svc.from("profiles").select("empresa_id").eq("id", userId).maybeSingle();
    const { data: roleRow } = await svc.from("user_roles").select("role").eq("user_id", userId);
    const isSuperAdmin = (roleRow ?? []).some((r: any) => r.role === "super_admin");
    if (!isSuperAdmin && profile?.empresa_id !== empresaId) {
