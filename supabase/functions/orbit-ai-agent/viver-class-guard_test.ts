...
const canonical = `Perfeito, {{nome}}. Aqui esta o link da aula ao vivo: https://meet.google.com/esz-wgwt-pge\n\nA aula sera na terca-feira, as 19:30.`;

Deno.test("aceite explícito após oferta de acesso entrega template canônico", () => {
  const messages = [
    { direcao: "OUT", mensagem: "Quer que eu te mande o link para participar da aula?", timestamp: "2026-08-25T18:00:00Z" },
...
Deno.test("sim para pergunta sem aula não dispara link", () => {
  const messages = [
    { direcao: "OUT", mensagem: "Você já trabalha com revendedoras?", timestamp: "2026-08-25T18:00:00Z" },
    { direcao: "IN", mensagem: "Sim", timestamp: "2026-08-25T18:01:00Z" },
  ];
...
Deno.test("confirmação vaga ou texto livre não é interpretado como aceite", () => {
  const messages = [{ direcao: "OUT", mensagem: "Quer receber o acesso da aula?" }];
  assertEquals(previousAssistantOfferedClassAccess(messages, "não sei"), false);
  assertEquals(previousAssistantOfferedClassAccess(messages, "tenho sim uma loja"), false);
});

Deno.test("extrai somente um Google Meet canônico", () => {
  assertEquals(extractCanonicalClassUrl(canonical), "https://meet.google.com/esz-wgwt-pge");
  assertEquals(extractCanonicalClassUrl("https://viverjoias.com.br/aula-gratis"), null);
  assertEquals(extractCanonicalClassUrl(`${canonical} https://meet.google.com/outro`), null);
});
...
Deno.test("preserva link canônico e respostas sem conteúdo de aula", () => {
  assertEquals(enforceCanonicalClassLink("Link da aula: https://meet.google.com/esz-wgwt-pge", canonical).changed, false);
  assertEquals(enforceCanonicalClassLink("Entendi. Qual é seu principal desafio?", canonical).changed, false);
});
...
Deno.test("controle temporal distingue antes, durante e depois da aula em São Paulo", () => {
  ...
  assertEquals(viverClassPhase(new Date("2026-08-25T22:30:00Z")), "in_progress");
...
Deno.test("durante a aula usa linguagem presente e o link canônico", () => {
  const text = buildCanonicalClassDelivery(canonical, "Franciane", new Date("2026-08-25T22:39:00Z"));
...
Deno.test("falha fechada se não houver autoridade válida para link de aula", () => {
  const guarded = enforceCanonicalClassLink("Link da aula: https://viverjoias.com.br/aula-gratis", null);
  assertEquals(guarded.changed, true);
