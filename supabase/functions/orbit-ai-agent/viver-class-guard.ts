...
const AFFIRMATIVE_RE =
  /^(?:(?:sim|s|quero|gostaria|pode|pode\s+sim|vou\s+sim|eu\s+vou|manda|envia|claro|com\s+certeza|tenho\s+(?:sim|interesse)|eu\s+quero|quero\s+sim)[.!?,\s]*|sim\b.{0,50}\b(?:quero|pode|liber\w*|mand\w*|envi\w*)\b.*|(?:eu\s+)?(?:quero|vou)\b.{0,50}\b(?:particip\w*|acesso|aula)\b.*|pode\b.{0,50}\b(?:liber\w*|mand\w*|envi\w*)\b.*)$/iu;
const GRATITUDE_PREFIX_RE =
  /^(?:(?:ah|aah)[,!]?\s*)?(?:obrigad[ao]|valeu|agrade[cç]o)[.!?,;:\s-]*/iu;
...
export function isExplicitClassAcceptance(text: string): boolean {
  const normalized = (text || "").trim().replace(GRATITUDE_PREFIX_RE, "")
    .trim();
