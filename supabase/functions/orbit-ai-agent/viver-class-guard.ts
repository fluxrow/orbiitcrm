...
const ACCESS_OFFER_RE = /(?:quer|gostaria|posso).{0,50}(?:link|acesso|participar)|(?:link|acesso).{0,40}(?:aula|participar)/iu;
const AFFIRMATIVE_RE = /^(?:sim|s|quero|gostaria|pode|pode\s+sim|manda|envia|claro|com\s+certeza|tenho\s+(?:sim|interesse)|eu\s+quero|quero\s+sim)[.!?,\s]*$/iu;
...
export function renderCanonicalClassTemplate(templateBody: string, name: string | null | undefined): string {
...
export function buildCanonicalClassDelivery(
  ...
  if (phase !== "in_progress") return renderCanonicalClassTemplate(templateBody, name);
  const canonicalUrl = extractCanonicalClassUrl(templateBody);
  if (!canonicalUrl) throw new Error("class_link_authority_missing");
  const safeName = (name || "").trim();
  return `${safeName ? `${safeName}, a` : "A"} aula já está acontecendo. Entre agora por este link: ${canonicalUrl}`;
}
...
export function enforceCanonicalClassLink(
  ...
  if (!CLASS_CONTEXT_RE.test(response || "")) return { text: response, changed: false };
  const links = (response.match(URL_RE) || []).map(cleanUrl);
  if (links.length === 0) return { text: response, changed: false };
  const canonicalUrl = extractCanonicalClassUrl(templateBody);
  if (!canonicalUrl) {
    return {
      text: "Não consegui confirmar o acesso da aula agora. Você quer que eu verifique isso antes de te enviar?",
      changed: true,
      reason: "class_link_authority_missing",
    };
  }
  if (links.every((link) => link === canonicalUrl)) return { text: response, changed: false };
  return {
