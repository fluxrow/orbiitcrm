...
function assertPublicHttpsUrl(raw: string): URL {
  ...
  if (
    ...
    (ipv4.some((part) => part > 255) || ipv4[0] === 10 || ipv4[0] === 127 ||
      ipv4[0] === 0 || (ipv4[0] === 169 && ipv4[1] === 254) ||
  ...
  if (
    host === "::1" || host.startsWith("fe80:") || host.startsWith("fc") ||
    host.startsWith("fd")
  ) {
    throw new Error("media_url_host_not_allowed");
  }
...
async function describeImage(
  ...
  if (!/^image\/(jpeg|png|webp|gif)$/i.test(mime)) {
    throw new Error(`image_mime_unsupported:${mime}`);
  }
  const result = await callAnthropic({
    system:
      "Descreva objetivamente a imagem recebida por um lead em uma conversa comercial. Não invente texto ilegível nem conclusões. Retorne somente a descrição útil em português do Brasil, em até 500 caracteres.",
    messages: [{
      role: "user",
      content: [
        {
...
serve(async (req) => {
  ...
  try {
    const { data: conversation } = await supabase.from("orbit_conversas")
      .select("id, prospect_id, human_talk, ai_processing").eq(
        "id",
        claimed.conversa_id,
      ).single();
    if (!conversation?.id || !conversation?.prospect_id) {
      throw new Error("conversation_not_found");
    }
    const { data: prospect } = await supabase.from("orbit_prospects").select(
      "id, telefone",
    ).eq("id", conversation.prospect_id).single();
    if (!prospect?.id) throw new Error("prospect_not_found");
    const { data: config } = await supabase.from("orbit_ai_config").select(
      "modo_automatico, inbound_image_understanding_enabled, inbound_audio_transcription_enabled, tts_api_key",
    ).eq("empresa_id", claimed.empresa_id).maybeSingle();
    const type = String(claimed.tipo_midia || "").toLowerCase();
    const enabled = type === "image"
      ? config?.inbound_image_understanding_enabled
      : type === "audio"
      ? config?.inbound_audio_transcription_enabled
      : false;
    if (!enabled) {
    ...
    const path =
      `${claimed.empresa_id}/inbound/${claimed.conversa_id}/${claimed.id}.${
        safeExtension(media.mime)
      }`;
    const { error: uploadError } = await supabase.storage.from("orbit-media")
    ...
    const audio = type === "audio"
      ? await transcribeAudio(media.bytes, media.mime, {
        tenantElevenLabsKey: config?.tts_api_key,
        globalElevenLabsKey: Deno.env.get("ELEVENLABS_API_KEY"),
    ...
    const extracted = type === "image"
      ? await describeImage(
        media.bytes,
        media.mime,
        String(claimed.mensagem || ""),
      )
      : audio!.text;
    const prefix = type === "image"
      ? "Imagem recebida"
      : "Transcrição do áudio recebido";
    const original = String(claimed.mensagem || "").replace(
      /^📎\s*(image|audio)$/i,
      "",
    ).trim();
    const agentText = `${
    ...
    if (!dry_run && config?.modo_automatico && !conversation.human_talk) {
      const { data: lock } = await supabase.from("orbit_conversas").update({
        ai_processing: true,
      }).eq("id", conversation.id).eq("ai_processing", false).select("id");
      if (lock?.length) {
        agentInvoked = true;
        const invokeAgent = fetch(
          `${supabaseUrl}/functions/v1/orbit-ai-agent`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
              Authorization: `Bearer ${serviceKey}`,
              "x-orbit-internal-secret":
                Deno.env.get("ORBIT_AI_AGENT_SECRET") || "",
            },
            body: JSON.stringify({
          ...
          console.error("[media-processor] agent invoke failed", error)
        );
        // Keep the worker alive until the downstream agent accepts the request.
        const edgeRuntime = (globalThis as any).EdgeRuntime;
        if (edgeRuntime) edgeRuntime.waitUntil(invokeAgent);
      }
    }
    return json({
      ok: true,
    ...
    await supabase.from("orbit_mensagens").update({
      ...
      media_processing_error: reason.slice(0, 500),
      media_processed_at: new Date().toISOString(),
    }).eq("id", message_id);
