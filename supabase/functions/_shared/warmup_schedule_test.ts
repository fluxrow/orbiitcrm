...
const day = (n: number) =>
  new Date(`2026-08-${String(10 + n).padStart(2, "0")}T15:00:00-03:00`);
...
const PROSPECTING_SOURCES = ["flow_initial", "flow_followup", "campaign"];
...
Deno.test("W5 D1 com 15 mensagens: 10 saem, 5 retidas por WARMUP_DAILY_LIMIT", () => {
  const items = Array.from({ length: 15 }, (_, i) => ({
    id: `msg-${i + 1}`,
    source_type: PROSPECTING_SOURCES[i % PROSPECTING_SOURCES.length],
  ...
  assertEquals(r.retained.map((x) => x.id), [
    "msg-11",
    "msg-12",
    "msg-13",
    "msg-14",
  ...
  assertEquals(
    new Set(r.retained.map((x) => x.reason)),
    new Set([RETAIN_REASON_DAILY]),
  );
});

Deno.test("W6 fontes de prospecção respeitam a cota diária", () => {
  for (const source of PROSPECTING_SOURCES) {
    const items = Array.from(
      { length: 3 },
      (_, i) => ({ id: `${source}-${i}`, source_type: source }),
    );
    const r = simulateWarmupBatch({
      items,
      config: {
    ...
    assertEquals(
      r.sent.length,
      0,
      `source ${source} furou a cota de prospecção`,
    );
...
Deno.test("W6b respostas e mensagens operacionais não consomem cota ou ritmo de prospecção", () => {
  const items = OPERATIONAL_SOURCES.map((source, i) => ({
    id: `op-${i}`,
    source_type: source,
  }));
  const r = simulateWarmupBatch({
  ...
  assertEquals(r.sent.length, items.length);
  assertEquals(r.retained.length, 0);
});
...
Deno.test("W7 ritmo por minuto retém com WARMUP_RATE_LIMIT", () => {
  ...
  assertEquals(r.retained.length, 1);
  assertEquals(
    new Set(r.retained.map((x) => x.reason)),
    new Set([RETAIN_REASON_RATE]),
  );
});
...
Deno.test("W8 next_attempt_at: minuto para rate, próximo dia para cota", () => {
  ...
  assertEquals(
    nextAttemptForRetain(RETAIN_REASON_RATE, now),
    new Date(now.getTime() + 60_000).toISOString(),
  );
  assertEquals(
    nextAttemptForRetain(RETAIN_REASON_DAILY, now),
