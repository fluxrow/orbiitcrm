...
Deno.test("controlled Viver outbox gate exposes the deploy version", () => {
  assertEquals(VIVER_CONTROLLED_OUTBOX_GATE_VERSION, "2026-09-21-v4");
});
...
Deno.test("pilot only admits explicitly marked controlled Viver operations to evidence checks", () => {
  ...
  assertEquals(PILOT_FOLLOWUP_INITIAL_DELIVERY_REQUIRED, "PILOT_FOLLOWUP_INITIAL_DELIVERY_REQUIRED");
  assertEquals(PILOT_FOLLOWUP_CANCELED_ON_REPLY, "PILOT_FOLLOWUP_CANCELED_ON_REPLY");
  assertEquals(PILOT_FOLLOWUP_CANCELED_AFTER_HUMAN_OUTBOUND, "PILOT_FOLLOWUP_CANCELED_AFTER_HUMAN_OUTBOUND");
