...
export function useUpdateMetaWhatsAppConfig() {
  ...
  return useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["orbit_meta_whatsapp_config"] }),
  });
}

export async function testMetaWhatsAppConnection(empresaId: string) {
  const { data, error } = await supabase.functions.invoke("orbit-meta-whatsapp-health", {
