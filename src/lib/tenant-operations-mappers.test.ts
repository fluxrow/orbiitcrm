...
const aggregate = {
  ...
  data: {
    ...
    queue: { pending_count: 3, processing_count: 2, failed_count: 1 },
    whatsapp: { connected: true, envio_real_liberado: false, queue_enabled: true },
...
describe("mapTenantOperationsPayload", () => {
  it("normaliza os nós do contrato agregador para os DTOs dos cartões", () => {
    ...
    expect(mapTenantOperationsPayload("whatsapp", aggregate)).toMatchObject({ active: true, real_send_enabled: false, sending_policy: { queue_enabled: true } });
